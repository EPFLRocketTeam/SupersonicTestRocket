...
bin_ends = np.array([0, 0.8, 1.2, 2])  # Mach number limits for regimes
...
if __name__ == "__main__":
    ...
    for i, (mach_start, mach_end) in enumerate(zip(bin_ends[:-1],
        ...
        print(f"Work done by drag accelerating between Mach {mach_start} "
              f"and {mach_end}: {work_bins_accel[i]:.2f} J")
    print("")
    ...
    for i, (mach_start, mach_end) in enumerate(zip(np.flip(bin_ends[:-1]),
        ...
        print(f"Work done by drag decelerating between Mach {mach_end} "
              f"and {mach_start}: "
    ...
    print("\nTotal work done by drag: "
          f"{np.sum(work_bins_accel + work_bins_decel):.2f} J")
